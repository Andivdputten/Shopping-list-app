...
function initApp() {
  ...
  if (
    ...
    throw new Error("HTML element missing. Check index.html IDs.");
  }
  ...
  async function handleDetectedBarcode(decodedText) {
    ...
    try {
      ...
      if (productData !== null) {
        ...
        const matchLabel =
          productData.name !== "" ? `External match found: "${productData.name}".` : "External match found.";
        ...
        setStatus(`${matchLabel} Review the autofill and save.`);
        return;
      }
  ...
  function findItemIndexByBarcode(barcode) {
    ...
    const index = items.findIndex((item) => {
      return item.barcode.trim() === normalizedBarcode;
    });
